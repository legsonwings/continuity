...
)
{
    ...
    if (gtid < num_prims)
    {
        ...
        verts[outv0idx].position = mul(mvpmatx, float4(in_vertices[inputvert_start].position, 1));
        verts[outv1idx].position = mul(mvpmatx, float4(in_vertices[inputvert_start + 1].position, 1));
    }
}
